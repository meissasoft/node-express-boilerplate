...
import tokenFixture from '../fixtures/token.fixture';
...
describe('Auth routes', () => {
  ...
  describe('POST /v1/auth/login', () => {
    test('should return 200 and login user if email and password match', async () => {
      ...
      expect(res.body.user).toEqual({
        ...
        isEmailVerified: userFixture.userOne.isEmailVerified,
      });
  ...
  describe('POST /v1/auth/logout', () => {
    test('should return 204 if refresh token is valid', async () => {
      ...
      const refreshToken = tokenService.generateToken(userFixture.userOne._id, expires, token.tokenTypes.REFRESH);
      await tokenService.saveToken(refreshToken, userFixture.userOne._id, expires, token.tokenTypes.REFRESH);
    ...
    test('should return 404 error if refresh token is blacklisted', async () => {
      ...
      const expires = moment().add(jwt.refreshExpirationDays, 'days');
      const refreshToken = tokenService.generateToken(userFixture.userOne._id, expires, token.tokenTypes.REFRESH);
      await tokenService.saveToken(refreshToken, userFixture.userOne._id, expires, token.tokenTypes.REFRESH, true);
  ...
  describe('POST /v1/auth/reset-password', () => {
    test('should return 204 and reset the password', async () => {
      ...
      const resetPasswordToken = tokenService.generateToken(
        userFixture.userOne._id,
        expires,
        token.tokenTypes.RESET_PASSWORD
      );
      await tokenService.saveToken(resetPasswordToken, userFixture.userOne._id, expires, token.tokenTypes.RESET_PASSWORD);
      ...
      const dbResetPasswordTokenCount = await Token.countDocuments({
        user: userFixture.userOne._id,
        type: token.tokenTypes.RESET_PASSWORD,
      });
  ...
  describe('POST /v1/auth/send-verification-email', () => {
    ...
    test('should return 204 and send verification email to the user', async () => {
      ...
      expect(sendVerificationEmailSpy).toHaveBeenCalledWith(userFixture.userOne.email, expect.any(String));
      const verifyEmailToken = sendVerificationEmailSpy.mock.calls[0][1];
      const dbVerifyEmailToken = await Token.findOne({ token: verifyEmailToken, user: userFixture.userOne._id });
    ...
    test('should return 401 error if access token is missing', async () => {
      ...
      await request(app).post('/v1/auth/send-verification-email').send().expect(UNAUTHORIZED);
    });
  });
  ...
  describe('POST /v1/auth/verify-email', () => {
    test('should return 204 and verify the email', async () => {
      ...
      const verifyEmailToken = tokenService.generateToken(userFixture.userOne._id, expires);
      await tokenService.saveToken(verifyEmailToken, userFixture.userOne._id, expires, token.tokenTypes.VERIFY_EMAIL);

      await request(app).post('/v1/auth/verify-email').query({ token: verifyEmailToken }).send().expect(NO_CONTENT);
      ...
      const dbVerifyEmailToken = await Token.countDocuments({
        user: userFixture.userOne._id,
        type: token.tokenTypes.VERIFY_EMAIL,
      });
    ...
    test('should return 400 if verify email token is missing', async () => {
      ...
      await request(app).post('/v1/auth/verify-email').send().expect(BAD_REQUEST);
    });
    ...
    test('should return 401 if verify email token is blacklisted', async () => {
      ...
      const expires = moment().add(jwt.verifyEmailExpirationMinutes, 'minutes');
      const verifyEmailToken = tokenService.generateToken(userFixture.userOne._id, expires);
      await tokenService.saveToken(verifyEmailToken, userFixture.userOne._id, expires, token.tokenTypes.VERIFY_EMAIL, true);

      await request(app).post('/v1/auth/verify-email').query({ token: verifyEmailToken }).send().expect(UNAUTHORIZED);
    });
...
describe('Auth middleware', () => {
  ...
  test('should call next with unauthorized error if access token is expired', async () => {
    await userFixture.insertUsers([userFixture.userOne]);
    const expires = moment().subtract(1, 'minutes');
    const accessToken = tokenService.generateToken(userFixture.userOne._id, expires, token.tokenTypes.ACCESS);
    const req = createRequest({ headers: { Authorization: `Bearer ${accessToken}` } });
    const next = jest.fn();
  ...
  test('should call next with no errors if user has required rights', async () => {
    ...
    const req = createRequest({
      headers: { Authorization: `Bearer ${userFixture.adminAccessToken}` },
      params: { userId: userFixture.userOne._id.toHexString() },
    });
    const next = jest.fn();

    await auth(...roles.roleRights.get('admin'))(req, createResponse(), next);
